...
**Note**: It takes some time for the excluded functions to disappear from the [Datadog serverless page][5] and [Datadog usage page][4]. Verify the filtering rules by checking the [`aws.lambda.invocations`][6] metric for the filtered functions. When Datadog stops monitoring a function, the value of `aws.lambda.invocations` falls to 0.

### UI

To use the UI to control which AWS Lambda functions Datadog is monitoring, navigate to the [AWS Integration page][7] and add tags as `key:value` sets to the **to Lambdas with tag:** field.
...
To use the API to control limit which AWS Lambda functions Datadog is monitoring, reference the [tag filter documentation][8].
...
[6]: https://app.datadoghq.com/metric/explorer?exp_metric=aws.lambda.invocations&exp_group=functionname&exp_agg=sum
